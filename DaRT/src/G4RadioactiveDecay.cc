            ...
            if (inputChars[0] != '#' && inputLine.length() != 0)
            {
                ...
                else if (found)
                {
                    ...
                    if (inputLine.length() < 72)
                    {
                        ...
                        break;
                        case BetaMinus:
                            modeTotalBR[BetaMinus] = decayModeTotal;
                            break;
                        case BetaPlus:
                            modeTotalBR[BetaPlus] = decayModeTotal;
                            break;
                        case KshellEC:
                            modeTotalBR[KshellEC] = decayModeTotal;
                            break;
                        case LshellEC:
                            modeTotalBR[LshellEC] = decayModeTotal;
                            break;
                        case MshellEC:
                            modeTotalBR[MshellEC] = decayModeTotal;
                            break;
                        case NshellEC:
                            modeTotalBR[NshellEC] = decayModeTotal;
                            break;
                        case Alpha:
                            modeTotalBR[Alpha] = decayModeTotal;
                            break;
                        case Proton:
                            modeTotalBR[Proton] = decayModeTotal;
                            break;
                        case Neutron:
                            modeTotalBR[Neutron] = decayModeTotal;
                            break;
                        case SpFission:
                            modeTotalBR[SpFission] = decayModeTotal;
                            break;
                        case BDProton:
                        ...
                        case Neutron2:
                            /* Not yet implemented */ break;
                        case Triton:
                            modeTotalBR[Triton] = decayModeTotal;
                            break;
                        case RDM_ERROR:
...
void G4RadioactiveDecay::DecayAnalog(const G4Track &theTrack)
{
    ...
    for (G4int index = 0; index < numberOfSecondaries; ++index)
    {
        G4Track *secondary = new G4Track(products->PopProducts(), finalGlobalTime,
                                         theTrack.GetPosition() + changePosition);
...
G4ThreeVector G4RadioactiveDecay::calculateDiffusion(const G4double diffusionTime, const G4double Dwater, const G4Track &theTrack)
{
    ...
    else
    {
        ...
        {
            // The diffusion path starts in the world and possibly intersects the seed
            // G4cout << "starts in world and might cross seed" << startVolume << G4endl;

            G4double distanceWater = abs(G4RandGauss::shoot(0, sqrt(6 * Dwater * diffusionTime / s)));

            // option 1 it starts outside and stops inside
            ...
            {
                // never reaches the seed
                changePosition = distanceWater * direction;
                // G4cout << "Never crosses seed stays in water" << startVolume << G4endl;
            }
            else
            {
                // does intersect the seed volume
                // calculate the time remaining for diffusion after reaching the seed
                // G4cout << "Diffusion would cross the seed" << G4endl;
