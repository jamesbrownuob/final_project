...
G4DNAIRT::G4DNAIRT() : G4VITReactionProcess(),
...
void G4DNAIRT::Sampling(G4Track *track)
{
    ...
    for (int ii = xiniIndex; ii <= xendIndex; ii++)
    {
        for (int jj = yiniIndex; jj <= yendIndex; jj++)
        {
            for (int kk = ziniIndex; kk <= zendIndex; kk++)
            {
                ...
                {
                    if (!spaceBin[n] || track == spaceBin[n])
                        continue;
                    if (spaceBin[n]->GetTrackStatus() == fStopButAlive)
                        continue;
                    ...
                    if ((molConfA->GetDiffusionCoefficient() == 0) && (molConfB->GetDiffusionCoefficient() == 0))
                        continue;


                    auto it = std::find(reactivesVector->begin(), reactivesVector->end(), molConfB);
                    if (it == reactivesVector->end())
                        continue;
    ...
    {
        if ((*fReactionDatas)[u]->GetReactant2()->GetDefinition() == G4Deoxyribose::Definition() || (*fReactionDatas)[u]->GetReactant2()->GetDefinition() == G4Guanine::Definition() || (*fReactionDatas)[u]->GetReactant2()->GetDefinition() == G4Thymine::Definition() ||
            (*fReactionDatas)[u]->GetReactant2()->GetDefinition() == G4Adenine::Definition() ||
            (*fReactionDatas)[u]->GetReactant2()->GetDefinition() == G4Cytosine::Definition())
            continue;
    ...
    if (index != -1)
    {
        // G4cout << "scavenged: " << minTime << '\t' << molConfA->GetName() << '\n';
...
G4double G4DNAIRT::GetIndependentReactionTime(const G4MolecularConfiguration *molA, const G4MolecularConfiguration *molB, G4double distance)
{
    ...
    if (reactionType == 0)
    {
        ...
        if (W > 0 && W < Winf)
            irt = (0.25 / D) * std::pow((r0 - sigma) / erfc->erfcInv(r0 * W / sigma), 2);
    ...
    else if (reactionType == 1)
    {
        ...
        if (sigma > r0)
        {
            if (fReactionData->GetProbability() > G4UniformRand())
                return 0;
            else
                return irt;
        }
                                                           ...
                                                           const G4Track &trackB)
{
    ...
    if (nbProducts)
    {
        ...
        const G4ThreeVector reactionSite = sqrD2 * inv_numerator * trackA.GetPosition() + sqrD1 * inv_numerator * trackB.GetPosition();
