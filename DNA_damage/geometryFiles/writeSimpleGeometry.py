...
nanometer = 1 # save in nm - convert to mm in geant
        ...
        for z in range(numLayers):
            for i in range(6):
                ...
                for j in range(len(posSugar1_x)):
                    ...
                    f.extend([posSugar1_x[j],posSugar1_y[j],posSugar1_z[j],posSugar2_x[j],posSugar2_y[j],posSugar2_z[j],posBase1_x[j], posBase1_y[j], posBase1_z[j], posBase2_x[j], posBase2_y[j], posBase2_z[j]])
                    

# plt.show()

output_file = 'sugarPos_{}x{}_{}nm.bin'.format(numRows,numColumns, volumeWidth)
out_file = open(output_file,"wb")
s = struct.pack('f'*len(f), *f)
out_file.write(s)
out_file.close()

f = []
for i in range(len(histonePositions)):
    f.extend([histonePositions[i][0],histonePositions[i][1],histonePositions[i][2],histonePositions[i][3],histonePositions[i][4],histonePositions[i][5]])

output_file = 'histonePositions_{}x{}_{}nm.bin'.format(numRows,numColumns, volumeWidth)
out_file = open(output_file,"wb")
s = struct.pack('f'*len(f), *f)
out_file.write(s)
out_file.close()