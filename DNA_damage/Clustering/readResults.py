...
def calcComplexDSB(SBtype: str, data: list, infoHeaderIndex: int = 0, infoStartIndex: int = 1, dataHeaderIndex: int = 2, dataStartIndex: int = 3) -> float:
...
def DSBClusterDistances(data: list, dataHeaderIndex: int = 2, dataStartIndex: int =3) -> list:
    """ Return the distance between DSB for all event where there are more than one DSB
    ...
    Returns:
        list: distances between DSB clusters for all events
    """

    distances = [literal_eval(a[data[dataHeaderIndex].index("Total_DSBdistances")].strip(' ').replace("\t", ",")) for a in data[dataStartIndex:]]

    distances = [a for sublist in distances for a in sublist]

    return distances

def DSB10kbpDistant(data: list, infoHeaderIndex: int = 0, infoStartIndex: int = 1, dataHeaderIndex: int = 2, dataStartIndex: int =3) -> list:
    """ Calculate the number of DSB (direct and indirect combined) which are seperated by more than 10kbp per Gy per Gbp
    ...
    Returns:
        list: number of DSB seperated by 10kbp
    """
    ...
    for e in distances:
        if e!=[]:
            positions = [0] #assume first DSB is at position 0, then add distance between DSB to get relative positions
            for val in e:
            ...
            if len(e)==1:
                ...
                for i in range(len(positions)-1):
                    if (positions[i]-positions[i-1]>=10000) or (positions[i+1]-positions[i]>=10000):
                        numDSBDistant+=1
                if (positions[-1]-positions[-2]>=10000):
                    numDSBDistant+=1
    totalDose = np.sum([float(a[data[dataHeaderIndex].index("DoseGy")]) for a in data[dataStartIndex:]])
    numGbp = float(data[infoStartIndex][data[infoHeaderIndex].index("numBP")])*1e-9

    return numDSBDistant/totalDose/numGbp